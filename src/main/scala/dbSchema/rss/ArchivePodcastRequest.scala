...
case class ArchivePodcastRequest(archiveIds: Seq[String], useArchiveOrder: Boolean = true, filePattern: String, podcastTemplate: Podcast) {
  ...
  def getPodcast(filePattern: String, useArchiveOrder: Boolean, podcast: Podcast): Podcast = {
    val podcasts = archiveIds.map(archiveId => {
      ...
      archiveItem.toPodcast(filePattern = filePattern, useArchiveOrder = useArchiveOrder, podcast = podcast)
    })
